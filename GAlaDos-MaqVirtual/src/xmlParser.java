...
public class xmlParser {
			...
			for (int temp = 0; temp < auxiliarLista.getLength(); temp++) { 
				...
				if (auxiliar.getNodeType() == Node.ELEMENT_NODE) { 
					...
					if(!eElement.getNodeName().equals("constantes")){
						...
						//Guardar globales
						NodeList auxiliarLista2 = eElement.getElementsByTagName("variablesGlobales");
						//System.out.println("auxLs"+auxiliarLista.getLength());
						auxiliar = auxiliarLista2.item(0);	
						if (auxiliar.getNodeType() == Node.ELEMENT_NODE) { 
							//Objeto elemento con los subnodos
							Element eElement2 = (Element) auxiliar;
							NodeList auxiliarLista3 = eElement2.getElementsByTagName("variableGlobal");
							//System.out.println("auxLs"+auxiliarLista.getLength());
							bloque.inicializarBase(1000);
							//Iterar la lista para agregar cada objeto
							for (int temp2 = 0; temp2 < auxiliarLista3.getLength(); temp2++) {
								auxiliar = auxiliarLista3.item(temp2);
								if (auxiliar.getNodeType() == Node.ELEMENT_NODE) { 
									//Obtener los datos de variables globales
									Element eElement3 = (Element) auxiliar;		
									
									int tipo = Integer.parseInt(eElement3.getElementsByTagName("tipo").item(0).getTextContent());
									int direccion = Integer.parseInt(eElement3.getElementsByTagName("direccion").item(0).getTextContent());
									
									switch(tipo){
									case 0:
										bloque.ingresaElementoEntero(bloque.mapearDireccion(direccion), -1);
										break;
									case 1:
										bloque.ingresaElementoDecimal(bloque.mapearDireccion(direccion), -1);
										break;
									case 2:
										bloque.ingresaElementoTexto(bloque.mapearDireccion(direccion), "");
										break;
									case 3:
										bloque.ingresaElementoBooleano(bloque.mapearDireccion(direccion), false);
										break;
									}
						...
						NodeList auxiliarLista4 = eElement.getElementsByTagName("variablesRetornos");
						//System.out.println("auxLs"+auxiliarLista.getLength());
						auxiliar = auxiliarLista4.item(0);	
						if (auxiliar.getNodeType() == Node.ELEMENT_NODE) { 
							//Objeto elemento con los subnodos
							Element eElement2 = (Element) auxiliar;
							NodeList auxiliarLista5 = eElement2.getElementsByTagName("variableRetorno");
							//System.out.println("auxLs"+auxiliarLista.getLength());
							
							//Iterar la lista para agregar cada objeto
							for (int temp2 = 0; temp2 < auxiliarLista5.getLength(); temp2++) {
	...
	/*
	 * public ArrayList<registro> cargarConstantes(ArrayList<registro>
					...
					if(!eElement.getNodeName().equals("constantes")){
						...
						if (auxiliar.getNodeType() == Node.ELEMENT_NODE) {
							...
							if (auxiliar.getNodeType() == Node.ELEMENT_NODE) {
									...
									funcion.setNombre(eElement3.getNodeName());
									...
									if (auxiliar.getNodeType() == Node.ELEMENT_NODE) {
										...
										for (int temp3 = 0; temp3 < auxiliarLista6
											...
											if (auxiliar.getNodeType() == Node.ELEMENT_NODE) {
												...
												int tipo = Integer.parseInt(eElement5.getElementsByTagName("tipo").item(0).getTextContent());
												int direccion = Integer.parseInt(eElement5.getElementsByTagName("direccion").item(0).getTextContent());
  ...
  public  bloque cargarConstantes(bloque bloque) {
	...
	for (int temp = 0; temp < nList.getLength(); temp++) { 
		...
		if (nNode.getNodeType() == Node.ELEMENT_NODE) { 
			...
			direccion = Integer.parseInt(eElement.getElementsByTagName("direccion").item(0).getTextContent());						
			//Obtener el tipo
			...
			switch (tipo) {
			case 0:				
				bloque.ingresaElementoEntero(bloque.mapearDireccion(direccion), Integer.parseInt(valor));				
				break;
			case 1:
				bloque.ingresaElementoDecimal(bloque.mapearDireccion(direccion), Double.parseDouble(valor));		
				break;
			case 2:
				bloque.ingresaElementoTexto(bloque.mapearDireccion(direccion),valor);		
				break;
			case 3:
				if(valor.equals("falso")){					
					bloque.ingresaElementoBooleano(bloque.mapearDireccion(direccion), false);		
				} else {
					bloque.ingresaElementoBooleano(bloque.mapearDireccion(direccion), true);	
				}
				break;			
			}			
		}
  ...
  public int tipoRetorno(String objeto, String direccion) {
	...
	NodeList variablesRetornos = eElement2.getElementsByTagName("variableRetorno");
	
	//Iterar la lista para agregar cada nodo al bloque
		for (int temp = 0; temp < variablesRetornos.getLength(); temp++) { 
	Element eElement = (Element) variablesRetornos.item(temp); 
	//Cargar variables locales
	if(eElement.getElementsByTagName("direccion").item(0).getTextContent()==direccion){
		return Integer.parseInt(eElement.getElementsByTagName("tipo").item(0).getTextContent());
	}	
		}
	
	 //tipo =  variablesLocales.getLength();
	    } catch (Exception e) {
	    	e.printStackTrace();
	    }
	    return tipo;
